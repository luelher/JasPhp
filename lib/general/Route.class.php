...
class Router {
  ...
  public function __construct() {
    ...
    $pos_script_name = strpos($request, $_SERVER['SCRIPT_NAME']);
    if($pos_script_name===0)
      $request = substr($request, $pos_script_name+strlen($_SERVER['SCRIPT_NAME']));
  ...
  private function set_route($route) {
    ...
    $this->action = isset($params['action']) ? $params['action'] : '' ; unset($params['action']);
